...
void ReplaceStep::replaceToken(const int a_index, const std::string& a_token, const std::string& a_filter, const std::string& a_value)
{
	m_configuration[a_index].token = a_token;
	m_configuration[a_index].filter = a_filter;
...
void ReplaceStep::save(QXmlStreamWriter& a_writer)const
{
	...
	for (const auto& token : m_configuration)
	{
		...
		a_writer.writeAttribute("Filter", QString::fromLatin1(token.filter));
